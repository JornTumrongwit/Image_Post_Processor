...
int main()
{
    ...
    if (bloom == true) {
        ...
        {
            FrameBuffer buffer = FrameBuffer(SCR_WIDTH, SCR_HEIGHT);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (bloom == true) {
            ...
            {
                glBindFramebuffer(GL_FRAMEBUFFER, pingpongBuffer[horizontal]->frameBuffer);
                shaderBlur.setInt("horizontal", horizontal);
                glBindTexture(
                    GL_TEXTURE_2D, first_iteration ? displayBuffer.texture : pingpongBuffer[!horizontal]->texture
                );
        ...
        glBindTexture(GL_TEXTURE_2D, pingpongBuffer[!horizontal]->frameBuffer);
        glDrawArrays(GL_TRIANGLES, 0, 6);
        glBindVertexArray(0);
