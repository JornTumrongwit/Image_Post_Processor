...
const vec3 magenta = vec3(0.75, 0.2, 0.4);
const vec3 green = vec3(0.6, 0.75, 0.2);
float cbrtf( float x )
{
...
vec3 okmagenta = sRGBtoOklab(magenta);
vec3 okgreen = sRGBtoOklab(green);
void main()
{
    FragColor = texture(texture1, TexCoord).rgb;
    vec3 oklabcol = sRGBtoOklab(FragColor);
    //oklabcol.r = floor(oklabcol.r * 16.0) / (16.0);
    float hue = atan(okmagenta.b, okmagenta.g);
    float c = sqrt(okmagenta.g * okmagenta.g + okmagenta.b * okmagenta.b);
    float greenhue = atan(okgreen.b, okgreen.g)*2.0;
    float thishue = atan(oklabcol.b, oklabcol.g)*2.0;
    oklabcol.g = abs(thishue - greenhue) < 0.5? c*cos(hue):oklabcol.g;
    oklabcol.b = abs(thishue - greenhue) < 0.5? c*sin(hue):oklabcol.b;
