...
const vec3 green = vec3(39.0/255.0, 66.0/255.0, 13.0/255.0);
const vec3 black = vec3(0.0, 0.0, 0.0);
const vec3 brown = vec3(48.0/255.0, 39.0/255.0, 39.0/255.0);
...
vec3 labbrown = sRGBtoOklab(brown);
...
{
    return vec3(lab.r, sqrt(lab.g*lab.g + lab.b * lab.b), atan(lab.b, lab.g));
}

vec3 LCHToOklab(vec3 LCH)
{
    return vec3(LCH.r, LCH.g*cos(LCH.b), LCH.g*sin(LCH.b));
}
void main()
{
    FragColor = texture(texture1, TexCoord).rgb;
    vec3 oklabcol = sRGBtoOklab(FragColor);
    vec3 lchcol = OklabToLCH(oklabcol);
    vec3 lchgreen = OklabToLCH(labgreen);
    lchcol.g *= 1.0-pow(distance(lchcol.b, lchgreen.b), 2);
    lchcol.r *= 1.0-pow(distance(lchcol.b, lchgreen.b), 10);
    lchcol.b = 1.0-pow(distance(lchcol.b, lchgreen.b), 2);
    oklabcol = LCHToOklab(lchcol);
