...
const vec3 magenta = vec3(54.0/255.0, 0.0, 59.0/255.0);
const vec3 black = vec3(0.0, 0.0, 0.0);
const vec3 blue = vec3(141.0/255.0, 228.0/255.0, 1.0);
float cbrtf( float x )
{
...
vec3 labmagenta = sRGBtoOklab(magenta);
vec3 labblue = sRGBtoOklab(blue);
float slope = 5.0;
float offset = 0.4;

float logisticgrowth(float x)
{
    return 1.0/(1.0+pow(2.71828,(-slope*(x-offset))));
}
void main()
{
    FragColor = texture(texture1, TexCoord).rgb;
    vec3 oklabcol = sRGBtoOklab(FragColor);
    oklabcol.r = logisticgrowth(floor((1.0-oklabcol.r) * 32.0) * 3.0 *(1.0-distance(vec2(0.5, 0.5), TexCoord.xy))/ (40.0));
    offset = 0.3;
    slope = 7.0;
    oklabcol.g = mix(labmagenta.g, labblue.g, logisticgrowth(oklabcol.r));
    oklabcol.b = mix(labmagenta.b, labblue.b, logisticgrowth(oklabcol.r));
